...
function Login() {
   useEffect(()=>{getUserData()},[])
    ...
    const getUserData = ()=>{
      axios.get("http://localhost:5001/api/userdetail").then((res)=>{setUserData(res.data.data)},(err)=>{alert(err)})
      
    }
    
    const loginCheck = ()=>{
      
      let user = userdata.filter((i)=> i.username == username)
      if(user.length>0){
  ...
  return (
    ...
    <div className='h-[90vh] w-[100%] flex justify-center items-center gap-10'>
     <div className='h-[80vh] w-[22%] border border-solid'>
     <img className='h-[80vh] w-[100%] object-cover object-center' src='https://images.ctfassets.net/az3stxsro5h5/NjZUwfga63k7mTZCeKhvH/2cbcc7fa1e907b0a9d37155077c253f8/Nov1-What_to_Post_on_Instagram_When_You_re_Fresh_Out_of_Ideas-Newsletter-Feature-FL'></img>
     </div>
     <div className='h-[80vh] w-[22%]  flex flex-col gap-4 '>
     <div className='h-[70vh] w-[100%] border-[1px] border-solid flex flex-col justify-center items-center gap-4'>
       <h1 className='text-[30px] w-[50%] '>Instagram</h1>
       <input onChange={(e)=>setUsername(e.target.value)} className='h-[vh] w-[80%] py-1 border-[1px] border-solid border-[gray] pl-[10px] text-[14px]'  placeholder='phone number. username ,or email'></input>
       <input onChange={(e)=>setPassword(e.target.value)} className='h-[vh] w-[80%] py-1 border-[1px] border-solid border-[gray] pl-[10px] text-[14px]'  placeholder='password'></input>
        <button onClick={()=> loginCheck()} className='h-[6vh] w-[80%] border border-solid rounded-[8px] bg-blue-400 font-bold text-[white]'>Log in </button>
     ...
     <div className='h-[10vh] w-[100%] flex justify-center items-center border-[1px] border-solid '>
        <button onClick={()=>navigate('/sign-up')}>Don't have an account?<span className='text-blue-600 font-bold '>Sign up</span> </button>
     </div>
     <label className='text-center'>Get tha app.</label>
     <div className='h-[10vh] w-[100%] flex gap-4 justify-center items-center'>
        <img  className='h-[6vh] w-[40%]'src='https://static.cdninstagram.com/rsrc.php/v3/yz/r/c5Rp7Ym-Klz.png'></img>
        <img  className='h-[6vh] w-[40%]'src= 'https://static.cdninstagram.com/rsrc.php/v3/yu/r/EHY6QnZYdNX.png'></img>
     </div>
     </div>
    </div>
